...
export async function extractTaittiriyaSamhita(targets?: string[]) {
  await ensureDir(EXTRACTED_DIR);
  await ensureDir(path.dirname(AUDIT_PATH));

  const records = [];
  for (const page of TTS_ACCENTED_PAGES) {
    const file = path.join(RAW_DIR, "accented", page.file);
    const raw = await fs.readFile(file, "utf8");
    records.push(...parseDumpRecords(readWikitextFromParseJson(raw)));
  }

  let prasnas = groupPrasnas(records, ttsSourceUrlForKanda);
  if (targets?.length) {
...
if (import.meta.main) {
  ...
  extractTaittiriyaSamhita(args.length ? args : undefined).catch((err) => {
    console.error(err);
    process.exit(1);
  });
}
