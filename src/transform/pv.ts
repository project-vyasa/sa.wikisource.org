...
import { ExtractedAdhyayaSchema, type ExtractedAdhyaya } from "../schema/pv";

/**
 * Stage 3: Transform extracted PV JSON into data/processed/panchavimsha-brahmana.
 */

const EXTRACTED_DIR = path.resolve("data/extracted/panchavimsha-brahmana");
const WORKSPACE_DIR = path.resolve("data/processed/panchavimsha-brahmana");
...
function emitSectionFile(adhyaya: ExtractedAdhyaya, sectionId: string, body: string, title: string | null): string {
  ...
  return (
    [emitContext(adhyaya, sectionId, title), emitBlock("p", n, body, "PV"), ""].join("\n").trimEnd() + "\n"
  );
}
...
export async function transformPanchavimshaBrahmana(targets?: string[]) {
...
if (import.meta.main) {
  ...
  transformPanchavimshaBrahmana(args.length ? args : undefined).catch((err) => {
    console.error(err);
    process.exit(1);
  });
}
